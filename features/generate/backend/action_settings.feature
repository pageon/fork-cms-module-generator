...
Feature: It is possible to generate a settings action
  Scenario: Generate a settings action
    When I run the command "generate:backend:action:settings" and I provide as input
      """
      TestModule[enter]MySettings[enter]Backend\Modules\TestModule\Domain\MySettings[enter]y[enter]firstName[enter][enter][enter]y[enter]lastName[enter][enter]1[enter][enter]
      """
    ...
    And the file "src/Backend/Modules/TestModule/Domain/MySettings/Command/SaveMySettingsHandler.php" should be dumped and look like "../generate/backend/resources/php71/Backend/Modules/TestModule/Domain/MySettings/Command/SaveMySettingsHandler.php"
